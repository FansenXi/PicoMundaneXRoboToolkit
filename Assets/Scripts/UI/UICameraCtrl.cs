...
public partial class UICameraCtrl : MonoBehaviour
{
    ...
    // Protocol selection: true for UDP, false for TCP
    [Header("Protocol")] 
    public bool useUdpProtocol = true;
    ...
    private void Awake()
    {
        ...
        // Use the appropriate camera handle based on protocol
        if (useUdpProtocol)
        {
    ...
    IEnumerator RequestCameraStreamCoroutine(string ip)
        {
            ...
            // Select protocol: UDP if useUdpProtocol is true, otherwise TCP
            var protocol = useUdpProtocol ? NetworkProtocol.UDP : NetworkProtocol.TCP;
            RemoteCameraWindowObj.GetComponent<RemoteCameraWindow>().StartListen(camPara.width, camPara.height, camPara.fps,
                camPara.bitrate, streamingPort, protocol);
                
            // Set the same protocol type in NetworkCommander to ensure consistency between sender and receiver
            if (NetworkCommander.Instance != null)
            {
                NetworkCommander.Instance.ProtocolType = protocol;
            }
