...
using XRoboToolkit.Network;
 // Import for NetworkProtocol enum
...
namespace Network
{
    ...
    public class NetworkCommander : MonoBehaviour
    {
        ...
        private NetworkProtocol _protocolType = NetworkProtocol.UDP;
        
        // Public property to get/set protocol type
        public NetworkProtocol ProtocolType
        {
        ...
        private class OpenCameraHandler : ICommandHandler
        {
            public void HandleCommand(byte[] data)
            {
                ...
                {
                    // Use appropriate camera handle based on protocol type
                    if (NetworkCommander.Instance.ProtocolType == NetworkProtocol.TCP)
                    {
        ...
        private class CloseCameraHandler : ICommandHandler
        {
            public void HandleCommand(byte[] data)
            {
                ...
                // Stop preview using the appropriate protocol handler
                if (NetworkCommander.Instance.ProtocolType == NetworkProtocol.TCP)
                {
